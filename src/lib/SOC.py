...
from common.common import soc_config, get_BMSmqtt   # ← from our fixed common.py
...
def save_state(estimator, path="soc_state.json"):
    try:
        log_soc.info("Saving SOC state")
        state = {
            "soc": estimator.soc,
            "last_time": time.time(),                    # always save current time
...
def load_state(estimator, path="soc_state.json"):
    if path not in os.listdir():
        return False
    try:
        log_soc.info("Loading SOC state")
        ...
        # Do NOT restore old last_time → avoid huge dt after reboot
        estimator.last_time = time.time()

        log_soc.info(f"SOC restored: {estimator.soc}%")
        return True
    except Exception as e:
...
class BatterySOC:
    def __init__(self, cfg=None):
        """cfg = optional soc_config instance (recommended)"""
        self.cfg = cfg or soc_config()          # safe fallback
    ...
    def update(self, current: float, voltage: float, temperature: float, nr_cells: int):
        """Main update method — call every sampling interval"""
        now = time.time()
        dt = max(0.01, now - self.last_time)          # prevent huge dt after reboot

        if self.num_cells != nr_cells or self.pack_table is None:
            self.num_cells = nr_cells
            self._build_pack_table()

        # Coulomb counting (FIXED sign!)
        ah_delta = (current * dt) / 3600.0
        coulomb_soc = self.soc + (ah_delta / self.cfg.capacity_ah) * 100.0
        ...
        # Relaxed state detection + blending
        ...
        if low_i and stable:
            if self.relaxed_start_time is None:
                ...
                coulomb_soc = coulomb_soc * 0.8 + ocv_soc * 0.2          # strong reset
        else:
            self.relaxed_start_time = None
            ocv_soc = self._interpolate_soc(ocv)
            coulomb_soc = coulomb_soc * 0.99 + ocv_soc * 0.01           # gentle correction
    ...
    def get_status(self):
        if (self.relaxed_start_time and
                time.time() - self.relaxed_start_time >= self.cfg.relaxed_hold_time):
            return "RELAXED"
        return "LOAD"
...
# =============================================================================
"""
# In main.py (after BMSmqtt and soc_config init)
soc_cfg = soc_config()
soc_cfg.init_mqtt_entities()          # ← only once, from main

soc_estimator = BatterySOC(soc_cfg)   # pass the same config instance
