...
class Relay:
    ...
    def on(self):
        """Turn the relay on"""
        self.log.info("Turning relay ON")
    ...
    def off(self):
        """Turn the relay off"""
        self.log.info("Turning relay OFF")
        if self._active_high:
            self._pin.value(0)
        else:
            self._pin.value(1)
    ...
    def toggle(self):
        ...
        self.log.info("Toggling relay state")
    ...
    def test(self, cycles: int = 5, on_time: float = 0.5, off_time: float = 0.5):
        ...
        """
        self.log.info("Starting relay test routine...")
        self.log.info(f"Pin: GPIO{self._pin}, Active {'HIGH' if self._active_high else 'LOW'}")
        self.log.info(f"Performing {cycles} cycles (ON {on_time}s / OFF {off_time}s)\n")
        ...
        self.log.info("Relay test completed successfully!")
